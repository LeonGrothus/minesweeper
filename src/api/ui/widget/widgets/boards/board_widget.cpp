...
BoardWidget::BoardWidget(const std::shared_ptr<Board2D> &board)
    : m_board(board), m_board_widget(std::make_shared<Board2dWidget>(board)), m_mine_count(board->get_mine_count()) {
    rebuild_layout();
}
...
void BoardWidget::rebuild_layout() {
    ...
    m_layout_widget = std::make_shared<EnumerationWidget>(inner_border, 1, m_x_spacing, m_y_spacing);
...
void BoardWidget::reset_game() {
    ...
    m_board_widget = std::make_shared<Board2dWidget>(m_board);
    m_board_widget->set_interactable(true);
    m_board_widget->set_x_spacing(m_x_spacing);
    m_board_widget->set_y_spacing(m_y_spacing);
