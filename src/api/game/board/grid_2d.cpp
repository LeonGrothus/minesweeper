...
template<typename T>
const Vector2D &Grid2D<T>::get_grid_size() const {
    return m_size;
}
...
template<typename T>
T &Grid2D<T>::get_at_index(const int index) {
    return m_data[index];
}

template<typename T>
const T &Grid2D<T>::get_at_index(const int index) const {
    return m_data[index];
}

template<typename T>
T &Grid2D<T>::get_at_pos(const Vector2D &pos) {
    return get_at_index(get_index(pos));
}
...
template<typename T>
std::vector<std::tuple<Vector2D, T> > Grid2D<T>::get_all_adjacent(const Vector2D &pos) {
    std::vector<std::tuple<Vector2D, T> > adjacent;
    for (int local_x = -1; local_x <= 1; local_x++) {
        for (int local_y = -1; local_y <= 1; local_y++) {
